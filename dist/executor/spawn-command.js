...
export function resolveWindowsComSpec(env) {
    ...
    if (lower.includes(".."))
        throw new Error(`unsafe ComSpec path: ${raw}`);
...
export function prepareSpawnCommand(argv, platform = process.platform, env = process.env) {
    ...
    const resolvedArgv = platform === "win32" ? [resolveSpawnBinary(argv[0], platform, env), ...argv.slice(1)] : argv;
    if (platform === "win32") {
